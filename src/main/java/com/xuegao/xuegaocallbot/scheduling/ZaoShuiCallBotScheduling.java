 ...
 */
@Component
public class ZaoShuiCallBotScheduling {
    private static final Logger log = LoggerFactory.getLogger(ZaoShuiCallBotScheduling.class);
    ...
    public void zaoShui() {
        ...
        log.info("[xuegao-call-bot][ZaoShuiCallBotScheduling][zaoShui][now={}]", now);
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("早睡提醒小助手\n");
        stringBuilder.append("现在时间为晚上 ");
        stringBuilder.append(LocalDateTime.now().getHour());
        stringBuilder.append(" 点\n");
        stringBuilder.append("建议早睡");
        List<String> userList = msgRangeUserId.getUserList();
        log.info("[xuegao-call-bot][ZaoShuiCallBotScheduling][zaoShui][userList={}]", JSON.toJSONString(userList));
        for (String userId : userList) {
            ...
            sendApplicationService.sendWeChatMessage(UserInfo.FJM, "1", "", "发送早睡运动成功" + userId, "0",
